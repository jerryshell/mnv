...
if os.getenv("FLATPAK_ID") ~= nil then
  ...
  vim.keymap.set("n", "<leader>ft", function()
    require("lazyvim.util").float_term(term_cmd, { cwd = require("lazyvim.util").get_root() })
  end, { desc = "Terminal (root dir)" })
  vim.keymap.set("n", "<leader>fT", function()
    require("lazyvim.util").float_term(term_cmd)
  end, { desc = "Terminal (cwd)" })
  -- gitui instead of lazygit
  local gitui_cmd = { "flatpak-spawn", "--host", "--env=TERM=xterm-256color", "gitui" }
  vim.keymap.set("n", "<leader>gg", function()
    require("lazyvim.util").float_term(gitui_cmd)
  end, { desc = "gitui (cwd)" })
  vim.keymap.set("n", "<leader>gG", function()
    require("lazyvim.util").float_term(gitui_cmd, { cwd = require("lazyvim.util").get_root() })
  end, { desc = "gitui (root dir)" })
end
